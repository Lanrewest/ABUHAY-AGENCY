...
export default function Sidebar({ open, onClose, role = 'buyer' }) {
  ...
  const menuItems = [
    ...
    { 
      ...
      icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>,
      roles: ['seller', 'agent']
    },
    { 
      name: 'My Listings', 
    ...
    },
    {
      ...
      icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>,
      roles: ['buyer']
    },
    { 
      name: 'Profile', 
  ...
  return (
    <aside className={`fixed md:static top-0 left-0 h-full min-h-screen w-64 bg-white text-gray-800 shadow-xl z-40 transform ${open ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 flex flex-col border-r border-gray-200`}>
      <div className="flex items-center justify-between px-6 py-6 border-b border-gray-200">
        <span className="text-xl font-extrabold tracking-tight text-blue-700">ABUHAY <span className="text-gray-600">Agency</span></span>
        <button onClick={onClose} className="text-blue-700 text-2xl font-bold md:hidden">&times;</button>
      </div>
